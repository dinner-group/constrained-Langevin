...
def rattle_kick(position, momentum, dt, potential, constraint, inverse_mass=None, energy=None, force=None, jac_constraint=None):
    ...
    b = np.pad(momentum - dt * force, (0, jac_constraint.shape[0]))
    x = np.linalg.solve(A, b)

    momentum_new = x[:momentum.size]
    lagrange_multiplier_new = x[momentum.size:]

    return position, momentum_new, lagrange_multiplier_new, energy, force, jac_constraint
...
def rattle_drift(position, momentum, lagrange_multiplier, dt, potential, constraint, inverse_mass=None, max_newton_iter=20, tol=1e-9):
    ...
    return position_new, momentum_new, lagrange_multiplier_new, jac_constraint, success

@partial(jax.jit, static_argnums=(5, 6))
def rattle_noise(position, momentum, dt, friction, prng_key, potential, constraint, inverse_mass=None, jac_constraint=None, temperature=1):
...
def gBAOAB(position, momentum, lagrange_multiplier, dt, friction, n_steps, thin, prng_key, potential, constraint, inverse_mass=None, energy=None, force=None, temperature=1, max_newton_iter=20, tol=1e-9):
    ...
    def cond(carry):
        i, position, momentum, lagrange_multiplier, energy, force, jac_constraint, out, success, prng_key = carry
    ...
    def loop_body(carry):
        ...
        position, momentum, _, energy, force, jac_constraint,= rattle_kick(position, momentum, dt / 2, potential, constraint, inverse_mass, energy, force, jac_constraint)
        position, momentum, lagrange_multiplier, jac_constraint, success =  rattle_drift(position, momentum, lagrange_multiplier, dt / 2, potential, constraint, inverse_mass, max_newton_iter, tol)
        position, momentum, _, prng_key = rattle_noise(position, momentum, dt, friction, prng_key, potential, constraint, inverse_mass, jac_constraint, temperature)
        position, momentum, lagrange_multiplier, jac_constraint, success =  rattle_drift(position, momentum, lagrange_multiplier, dt / 2, potential, constraint, inverse_mass, max_newton_iter, tol)
        position, momentum, _, energy, force, jac_constraint = rattle_kick(position, momentum, dt / 2, potential, constraint, inverse_mass, jac_constraint=jac_constraint)
        ...
        return (i + 1, position, momentum, lagrange_multiplier, energy, force, jac_constraint, out, success, prng_key)

    init = (0, position, momentum, lagrange_multiplier, energy, force, jac_constraint, out, True, prng_key)
    i, position, momentum, lagrange_multiplier, energy, force, jac_constraint, out, success, prng_key = jax.lax.while_loop(cond, loop_body, init)
